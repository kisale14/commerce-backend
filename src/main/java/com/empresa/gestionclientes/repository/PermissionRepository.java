...
public class PermissionRepository {
    ...
    public List<Permission> listarPermisos() {
        String sql = "SELECT id, nombre, descripcion, creado_en FROM permisos ORDER BY nombre ASC";
        return jdbcTemplate.query(sql, permissionRowMapper);
    }

    public Optional<Permission> buscarPorId(UUID id) {
        String sql = "SELECT id, nombre, descripcion, creado_en FROM permisos WHERE id = ?";
    ...
    public UUID guardar(String nombre, String descripcion) {
        String sql = "INSERT INTO permisos (nombre, descripcion) VALUES (?, ?) RETURNING id";
    ...
    public void actualizar(UUID id, String nombre, String descripcion) {
        String sql = "UPDATE permisos SET nombre = ?, descripcion = ? WHERE id = ?";
